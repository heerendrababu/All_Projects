...
//        // No response body, Postman will show status 200 OK
//    }
    ...
    public ResponseEntity<Void> createUser(@Valid @RequestBody User user) {
       ...
       // return ResponseEntity.status(HttpStatus.CREATED).build(); // Status 201 Created  
    }
...
//    }
    ...
    public ResponseEntity<?> getAll()
    {
        ...
        if (users.isEmpty()) 
        {
            //  Return 404 Not Found with a meaningful message when no data is found.
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
...
return ResponseEntity.status(HttpStatus.CREATED).body(response.toString(4)); // print JSON with 4 spaces
 ...
 if i want to return only success status code without body
